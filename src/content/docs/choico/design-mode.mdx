...
<span class="kbd">Design a game</span> from the Home Page. By making this
selection, you enter ChoiCo's design mode. The design mode consists of four
separate tabs that represent the different elements of the game you will be
designing.
...
If you want to change the background of the scene, click on <img class="!inline align-middle" src="/docs/img/scene-selection-icon.png" width="28" />
...
To add a new layer, click on <span class="kbd">Select File</span> and select an image from your
...
If you want to change the name of a layer, select it from the list of layers and press <span class="kbd">Rename Layer</span>.
Then, type the new name you want. The "Main" layer cannot be renamed.

**Delete layer**

In order to delete a layer, select it from the list of layers and press <span class="kbd">Delete Layer</span>.
...
To **add** new points to the scene of any layer, click on <img class="!inline" src="/docs/img/addPoint.png" width="28" />
...
to reclick on <img class="!inline" src="/docs/img/addPoint.png" width="28" />
the to return to the original state of the scene.

To **delete** a point you should right-click on it and select <span class="kbd">Delete point</span>.
...
position within the scene. To move it to a different layer, right-click on it and select <span class="kbd">Point Settings</span>.
In the window that appears there is a list of available layers and the layer the point belongs to is selected. You can
select another layer and click <span class="kbd">OK</span>.
The point will automatically be moved to the selected layer.
...
To **add** a new attribute for the game points, you need to click on the board’s <img class="!inline align-text-bottom" src="/docs/img/add.png" width="28" />
...
Select the property you wish to change and then click on <img class="!inline align-text-bottom" src="/docs/img/settings.png"width="28"/>.
...
-   **Image**: Allows you to add an image from your device by pressing the <span class="kbd">Choose</span> button
    ...
    **Formulas examples**:
    ...
    -   The value of an attribute can be written in relation to the
        ...
        **Money** and **Fun**, the **Fun** field can contain the formula:
        <code>money\*2</code>, so the value of the variable **Money** will be
        used.
...
property from the box next to its name and then click on <img class="!inline align-text-bottom" src="/docs/img/settings.png" width="28"/>,
...
<Aside>
	...
	blocks must be within the{' '}
	<code class="rounded !bg-[#995ba4] text-white">On game start</code> block.
	You must set the initial value for each numeric field you have created in
	the game.
</Aside>
...
example, after the player makes a selection, the game will check if <code class="rounded !bg-[#a35c7f] text-white">Covid&#9913;Risk</code>
is greater than or equal to <code>70</code> and, if it is, a pop-up window will appear with
the message _"You got close to a confirmed Covid case. Please be careful!!!!!"_. After
...
In the example below, the code checks if the selected field is Enter Home, <code class="rounded !bg-[#5b80a5] inline-block py-2 text-white"> if selectedChoice.name == <code class="rounded !bg-[#bdccdb] text-neutral">Enter Home</code></code>
and if it is, sets the active layer to <code class="rounded !bg-[#dbd4bd] text-neutral">bedroom</code>.
...
After, it checks if <code class="rounded !bg-[#a35c7f] text-white">Covid\*Risk</code>
is above <code>90</code> and if so, a pop-up window appears with the message
_"You need to work out..."_ and the game ends.
Then checks if <code class="rounded !bg-[#a35c7f] text-white">Covid_Risk</code> is <code>90</code> and if so,
a pop-up window appears with the message<em>"You've got Covid! Game is over"</em> and the game ends.
...
<Aside>
	You <b>must</b> use the{' '}
	<code class="rounded !bg-[#a35c7f] text-white">GameOver</code> block to end
	the game.
</Aside>
...
-   play it directly, by clicking <span class="kbd bg-green-700 text-white">Play</span> (figure above), without saving
    it, continuing if you wish to edit it afterwards.
-   save it on your device as a file with a <code>.choico</code> format by clicking <span class="kbd">Save</span>.
